...
export function newSupportTicket(
  req: { input_customerLinkID; input_userLinkId; input_title; input_details },
  res
) {
  try {
    const newTicket = {
...
export function getSupportTicketsByUser(req: { userID }, res) {
  try {
    const filter_supportTikets = supportTikets.find(
      (newSupportTicket) => newSupportTicket.userLinkId === req.userID
    );
